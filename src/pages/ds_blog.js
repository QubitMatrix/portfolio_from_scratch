...
export default function DSBlog (){
    return(<div id="ds-blog-mid">
        ...
        <ul>
            ...
            <li><u>Splay Tree:</u><br/>A structure which can be used to move the most recent search to the top thus making it easier to search it in the future.<br/>E.g.:In hospital records: a record which is searched once will be needed more frequently than those of discharged patients so moving this to a more easier to searc position reduces the time for search queries.</li><br/>
            <li><u>BST:</u><br/>A binary tree in which the root is greater than each left subtree and lesser than each right subtree makes a binary search tree.Inserting is time comsuming but search complexity is logarithmic so it is used for scenarios with few insertions and multiple searches.</li><br/>
            <li><u>Trie Tree:</u><br/>Trie tree (also known as prefix tree or digital tree) is a type of <a href="#">k-ary</a> search tree which can be used for searching keys.<br/>Using BST to search for a word of length M in a set of N keys then time time complexity would be O(M*log N)[for a well balanced BST], but using Trie we can bring it down to O(M). But at what cost do we get this benefit? The space complexity of a Trie. While most structures have linear or logarithmic space trie trees have quadratic space complexity because of all the pointers it holds for each key.<br/>It can be used for autocorrect features, dictionaries and substring matching.</li><br/><img src={require("../assets/trie.png")} id="trie-img" alt="trie"/><br/><br/><br/>
        ...
        Data structures is a very vast topic and as stated earlier I have tried my best to keep it short and to the point but data structures have a lot more to offer and everything mentioned above is just a bird's view of the vast world of data structures.<br/>
